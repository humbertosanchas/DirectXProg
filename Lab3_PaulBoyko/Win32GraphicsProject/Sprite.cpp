...
Vector2 Sprite::GetExtents()
{
	Vector2 extents((textureRegion.right - textureRegion.left) * scale * 0.5f, (textureRegion.bottom - textureRegion.top) * scale * 0.5f);
	...
	if (rotation != 0)
	{
		...
		Vector2 rotated = Vector2(extents.x * cosTheta - extents.y * sinTheta, extents.x * sinTheta + extents.y *cosTheta);
