...
{
	DirectX::FXMVECTOR clearColour = { 0.0f, 0.0f, 0.3f, 1.0f };

	timer.CheckTime();      //checking the time
	DeviceContext->ClearRenderTargetView(RenderTargetView, (const FLOAT*) &clearColour);    // clear the back buffer with the indicated colour
	if( depthStencilUsed )
	{
	...
	SwapChain->Present(1, 0);   // Swap buffers
}
...
{
	ID3D11Texture2D* pBackBuffer;         // pointer variable to the back buffer
	// setup the backbuffer as the rendering target
	this->SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pBackBuffer);    // get the address of the back buffer and store it in pBackBuffer
	if( FAILED(D3DDevice->CreateRenderTargetView(pBackBuffer, NULL, &this->RenderTargetView) ) )      // create a rendering target using the backbuffer
